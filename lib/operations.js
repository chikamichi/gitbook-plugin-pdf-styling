...
Operations.prototype.identifyFormat = function() {
  ...
  .then(function(output) {
    // I used to pipe the "identify" command to "| tail -n1" but it doesn't
    // return any output on gitbook.com's stack (dunno why).
    return output.split(/\r?\n/)[0];
  });
};

Operations.prototype.convertCmdFlags = {
  offset: function(size, color) {
    return '-bordercolor ' + color + ' -border ' +  size + ' -bordercolor none';
  }
};

Operations.prototype.bgAsNewPage = function(geometry, inputImagePath, options) {
  this.book.log.debug.ln('bgAsNewPage', arguments);

  // TODO: it's starting to look terrible already, time for commands factories.
  var command = [];

  command.push('convert');
  command.push(path.join(stringUtils.escapeSpaces(this.book.root), inputImagePath));

  if (options.offset)
    command.push(this.convertCmdFlags.offset(options.offset, options.backgroundColor));

  command.push('-gravity ' + this.im.toGravitySetting(options.position));
  command.push('-extent ' + geometry);
  command.push(path.join(this.book.options.pdfStyling.output, 'bg.pdf'));

  return execAsync.call(this.book, command);
};
